...
from config.bilibiliConfig import BilibiliCrawlerConfig
...
class BilibiliCrawler(BaseCrawler):
    ...
    def _process_search(self, target: str):
        ...
        try:
            ...
            self._save_videolist(search_list, target)
            ...
            for index, item in enumerate(search_list):
                ...
                logger.info(f"======[{self.config.PLATFORM}]第{index+1}个视频开始处理======")
                ID = item['id']
                if ID:
                    self._process_video(ID)
                    logger.info(f"======[{self.config.PLATFORM}]第{index+1}个视频处理完成======")
                else:
                    logger.error(f"******[{self.config.PLATFORM}]第{index+1}个视频处理异常******")
                    continue # 等全部处理完
    ...
    def _process_video(self, target: str):
        ...
        try:
            ...
            # 构造下载请求
            download_url = self.config.VIDEO_RES_URL.format(aid, cid)
            ...
            # 下载并保存视频
            self._download_video_bilibili(target, download_url,self.config.OUTPUT_VIDEOMP4_DIR)
    ...
    def _download_video_bilibili(self, target, download_url: str, save_dir: str = "./videos"):
        try:
            # 创建保存目录
...
if __name__ == "__main__":
    config = BilibiliCrawlerConfig()

    def test_search():
        """正常搜索测试"""
        try:
            crawler = BilibiliCrawler(headless=True, config = config)
